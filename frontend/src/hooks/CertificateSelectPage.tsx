...
const CertificateSelectPage = () => {
	...
	const handleCertSelect = (cert: Certificate, tabindex: number) => {
		...
		const utt = new SpeechSynthesisUtterance(
			`${cert.nameKo}을 선택하셨습니다. 발급하시겠습니까?`,
		);
		utt.lang = "ko-KR";
	...
	return (
		<Layout
			currentStep={2}
			/* 모달을 Layout의 modal 슬롯에 전달 → 키오스크 안에 표시됨 */
			modal={
				<Modal
					isOpen={isModalOpen}
					title="증명서 발급 확인"
		...
		>
			<PageWrapper>
				...
				{/* 보기 전환 버튼 — certificate 그룹 tabIndex 40, 41 */}
				<ViewToggle>
					<ToggleBtn
						...
						tabIndex={40}
						data-ttsmsg="카테고리 검색. 카테고리별로 증명서를 찾습니다."
					...
					<ToggleBtn
						...
						data-tabgroup="certificate"
						tabIndex={41}
						data-ttsmsg="명칭 검색. 증명서 이름으로 검색합니다."
				...
				{viewMode === "category" && (
					<ContentArea>
						{/* CategoryTabs: certificate 그룹 tabIndex 50+ (내부 정의) */}
						<CategoryTabs
							categories={categories}
							activeCategory={activeCategory}
							onSelect={(cat) => {
								setActiveCategory(cat);
								// 카테고리 변경 시 증명서 목록 첫 번째로 포커스 초기화
								setTimeout(
									() =>
										fm.initTabFocus(
											document,
