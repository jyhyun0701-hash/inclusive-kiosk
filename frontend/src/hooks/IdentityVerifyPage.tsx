...
const IdentityVerifyPage = () => {
  ...
  /* ── 지문 단계 진입 ── */
  useEffect(() => {
    if (verifyStep !== 'fingerprint') return;
    ...
    // 단계 전환 안내 (data-ttsmsg 보다 먼저 발화해야 자연스러움)
    speakSafe('주민번호가 확인되었습니다. 지문 인식을 진행해주십시오.');
  ...
  const handleScan = () => {
    ...
    setTimeout(() => {
      ...
      if (ok) {
        ...
        // 성공 → 발급 화면으로 이동 안내
        speakSafe('지문 인식 성공. 발급 화면으로 이동합니다.');
        setTimeout(() => {
          navigate('/issue', { state: { selectedCertificate: selectedCert } });
        }, 1800);
      } else {
        ...
        {verifyStep === 'fingerprint' && (
          <FPSection>
            ...
            <SMsg $s={scanStatus} role="status" aria-live="assertive">
              ...
              {scanStatus === 'success'  && '지문 인식 성공. 발급 화면으로 이동합니다.'}
              {scanStatus === 'failed'   && '지문 인식 실패. 다시 시도해주십시오.'}
            </SMsg>
            <ScanArea
...
const FPSVG = ({ sc }: { sc: boolean }) => (
  <svg width="68" height="68" viewBox="0 0 80 80" fill="none" aria-hidden="true">
    {["M40 10 C25 10 13 22 13 40 C13 52 18 62 26 68",
      ...
      <path key={i} d={d} stroke={sc ? '#4A90D9' : '#B8C4E0'} strokeWidth="3" strokeLinecap="round" fill="none"/>
    ))}
  </svg>
);
